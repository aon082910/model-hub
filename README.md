...
- **Thumbnails**: `app/thumbnails.py` via trimesh (headless render, matplotlib fallback); also computes volume/watertightness for print estimates. `.3mf` files use their own embedded slicer-rendered preview when present (skips mesh parsing for the thumbnail entirely); `is_watertight` is skipped past `MAX_WATERTIGHT_CHECK_FACES` since it scales with face count and multi-plate/multi-object project files (Bambu Studio/OrcaSlicer) concatenated into one mesh can reach several million faces
